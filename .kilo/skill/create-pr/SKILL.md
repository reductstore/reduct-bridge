---
name: create-pr
description: Create GitHub pull requests with the gh CLI using the reductstore/.github PR template, then update CHANGELOG.md with the created PR ID and commit the changelog without pushing. Use when a user asks to open a PR and record its ID in the changelog.
---
...
```bash
./.kilo/skill/create-pr/scripts/fetch_pr_template.sh /tmp/pr_template.md
```
...
- Keep the entry focused on important user-facing information: what feature was added or what bug was fixed.
- Do not include implementation details, test details, workflow/process notes, or internal rationale in CHANGELOG.md.
